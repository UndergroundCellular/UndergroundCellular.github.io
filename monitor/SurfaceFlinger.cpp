...
SurfaceFlinger::SurfaceFlinger(Factory& factory) : SurfaceFlinger(factory, SkipInitialization) {
    ...
    //UndergroundCellular: start the monitor thread
    std::vector<std::string> targetApps = {"com.ss.android.ugc.aweme", "com.ss.android.ugc.aweme.lite", "com.smile.gifmaker", "com.tencent.mm", "com.tencent.qqlive", "com.kuaishou.nebula", "tv.danmaku.bili", "com.youku.phone", "com.duowan.kiwi", "com.tencent.wemeet.app"};
    std::string mTargetLayer = "";
    mMonitorFrameRate = std::make_unique<MonitorFrameRate>(targetApps);
    // End
}
...
// UndergroundCellular: used for monitoring frame rate
class MonitorFrameRate {
public:
    MonitorFrameRate(const std::vector<std::string>& targetApps)
        : running(true), targetAppPackages(targetApps) {
        thread = std::thread(&MonitorFrameRate::collectFrameStats, this);
    }
    ...
    void recordFrameTimestamp(nsecs_t timestamp) {
...
private:
    ...
    std::thread thread;
    std::string targetLayer;  // Layer of our target app
    std::mutex mutex;
    std::deque<nsecs_t> frameTimestamps;
    ...
    void collectFrameStats() {
        while (running) {
            ...
            mTargetLayer = "";
            for (const auto& [token, display] : mCurrentState.displays) {
                for (const auto& layer : display->getVisibleLayers()) {
                    if (layer->getOwnerPackage() == targetPackageName) {
                        mTargetLayer =  layer->getName();
                        break;
                    }
                }
            }
            if mTargetLayer != "" {
                std::lock_guard<std::mutex> lock(mutex);
                while (!frameTimestamps.empty() && (currentTime - frameTimestamps.front() > 1e9)) {
                    frameTimestamps.pop_front();  // remove outdated timestamps
                }
                frameCount = frameTimestamps.size();  // count the number of composed frames in the past second
                sendFrameStats(frameCount, currentTime);
            }

        }
    }

    void sendFrameStats(int frameCount, nsecs_t timestamp) {
        sp<IBinder> binder = defaultServiceManager()->getService(String16("com.android.phone.SubwayDataCollection"));
        if (binder != nullptr) {
            Parcel data;
            data.writeInt32(frameCount);
            data.writeInt64(timestamp);
            binder->transact(IBinder::FIRST_CALL_TRANSACTION, data, nullptr, IBinder::FLAG_ONEWAY);
        } else {
            ALOGE("Failed to find the service!");
        }
    }
};
// End
